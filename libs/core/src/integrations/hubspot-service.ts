...
import { AppError, ErrorCode, ErrorCategory } from '../index';
...
export class HubSpotService {
  ...
  static async syncFromHubSpot(integrationId: string) {
    ...
    const lastSync = (integration.settings as any)?.lastSync || '1970-01-01T00:00:00.000Z';
    ...
    for (const contact of contacts) {
        ...
        await prisma.lead.upsert({
            where: { email_organizationId: { email: contact.properties.email, organizationId: integration.organizationId } },
            create: {
  ...
  static async syncToHubSpot(leadId: string) {
    ...
    const integration = lead.organization.integrations.find(i => i.provider === 'HUBSPOT' && i.isActive);
