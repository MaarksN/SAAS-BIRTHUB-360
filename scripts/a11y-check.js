...
async function runAudit() {
  ...
  try {
    ...
    const urls = ['http://localhost:3000', 'http://localhost:3000/dashboard'];
    ...
    for (const url of urls) {
      ...
      try {
        ...
        if (results.issues.length > 0) {
          console.error(
            `[FAIL] ${url}: Found ${results.issues.length} issues.`,
          );
        ...
        console.warn(
          `Could not test ${url} (Service might be down):`,
          e.message,
        );
      }
    }
    ...
    if (e.code === 'MODULE_NOT_FOUND') {
      console.warn(
        '[WARN] `pa11y` module not found. Skipping automated audit.',
      );
