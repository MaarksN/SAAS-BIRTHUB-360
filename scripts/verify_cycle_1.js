...
const checks = [
  {
    ...
    check: (content) => content.includes('export interface ICNPJEnrichmentResult'),
  ...
  },
  {
    ...
    check: (content) => content.includes("export * from './types/ldr';") && content.includes("export * from './types/pipeline';"),
    msg: 'libs/core/src/index.ts should export ldr and pipeline types'
  },
  {
    file: 'apps/web/services/ldr.service.ts',
    check: (content) => content.includes("from '@salesos/core';") && !content.includes("from '../types/ldr';"),
    msg: 'apps/web/services/ldr.service.ts should import from @salesos/core'
  },
  {
    file: 'apps/web/services/pipeline.service.ts',
    check: (content) => content.includes("from \"@salesos/core\";") || content.includes("from '@salesos/core';"),
  ...
  },
  {
    ...
    check: (content) => content.includes("from '@salesos/core';") && !content.includes('export interface Deal {'),
