...
export class SDRService {
  ...
  async handleObjection(text: string): Promise<IObjectionResponse> {
    const objectionsMap: Record<string, string> = {
      ...
      concorrente:
        'Eles são generalistas. Nós somos especialistas no seu nicho com [Feature X].',
    };
  ...
  async killObjectionPrice(): Promise<string> {
    return 'Se o preço fosse zero, a solução resolveria seu problema? Se sim, o problema é ROI, não custo. Vamos calcular o retorno?';
  }
  async killObjectionCompetitor(competitor: string): Promise<string> {
    return `O ${competitor} é ótimo para [Caso A]. Nós fomos desenhados especificamente para [Caso B - Sua Força]. É isso que você busca?`;
  }
  async killObjectionTime(): Promise<string> {
    return 'Entendo. A maioria dos meus melhores clientes também estava sem tempo. Justamente por isso implementaram nossa automação. Quer ver como economizar 10h/semana?';
  }

  // 7. Follow-up Builder
  async generateFollowUp(context: string): Promise<string> {
    return `Olá [Nome], pensando na nossa conversa sobre ${context}, vi este artigo e lembrei de você. Abs.`;
  }

  // 8. Break-up Email
  async generateBreakUp(): Promise<string> {
    return 'Olá [Nome], não quero ser o chato da caixa de entrada. Vou assumir que [Projeto] não é prioridade agora. Vou encerrar meu contato por aqui. Se algo mudar, estou à disposição.';
  }

  // 9. Gatekeeper Bypass
  async bypassGatekeeper(role: string): Promise<string> {
    return `Olá, sou [Nome]. Poderia me ajudar? Estou tentando enviar um material técnico para o Diretor sobre [Tópico], mas não queria incomodar no email errado. Qual a melhor forma?`;
  }
