...
export default function ToolPage() {
  ...
  const runIA = async () => {
    ...
    const emptyFields = tool.fields.filter(
      (f) =>
        f.type !== 'select' &&
        !f.placeholder?.includes('Opcional') &&
        !formValues[f.id],
    );
          ...
          <Card className={`border-t- border-t-4${tool.color}-500 shadow-lg`}>
            <CardContent className="p-6">
              <div className="mb-6 flex items-center gap-4">
                <div
                  className={`bg- size-12 rounded-xl${tool.color}-100 text- flex items-center justify-center${tool.color}-600`}
                >
          ...
          <div className="relative flex h-full min-h-[500px] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/50 p-4">
              <div className="flex items-center gap-2">
                <div
                  className={`size-2 rounded-full ${output ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]' : 'bg-slate-300'}`}
                ></div>
            ...
            <div className="custom-scrollbar flex-1 overflow-y-auto bg-slate-50/30 p-8">
              {loading ? (
                <div className="flex h-full animate-pulse flex-col items-center justify-center space-y-4 text-center">
                  <div
                    className={`border-t- size-16 rounded-full border-4 border-slate-100${tool.color}-500 animate-spin`}
                  ></div>
                ...
                <div className="flex h-full flex-col items-center justify-center space-y-4 text-center opacity-40">
                  ...
                  <p className="text-sm font-medium text-slate-500">
                    Preencha o formulário para gerar o conteúdo.
                  </p>
                </div>
              )}
            </div>
