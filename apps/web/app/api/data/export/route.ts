...
export const GET = createApiHandler(async (req) => {
  ...
  if (!['leads', 'deals'].includes(type) || !['csv', 'json'].includes(format)) {
     throw new AppError('Invalid parameters. Type must be leads/deals. Format must be csv/json.', 400, ErrorCode.INVALID_INPUT, ErrorCategory.VALIDATION);
  }

  const data = await DataManagementService.exportData(orgId, type, format);

  const headers = new Headers();
  headers.set('Content-Disposition', `attachment; filename="${type}-${new Date().toISOString()}.${format}"`);
