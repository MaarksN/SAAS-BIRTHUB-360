      ...
      {threads.map((thread) => (
        ...
        >
          <div className="mb-1 flex justify-between">
            <span
              className={`truncate text-sm font-semibold ${thread.isUnread ? 'text-white' : 'text-slate-400'}`}
            >
          ...
          <div
            className={`mb-1 truncate text-sm ${thread.isUnread ? 'font-medium text-slate-200' : 'text-slate-400'}`}
          >
            {thread.subject}
          </div>
