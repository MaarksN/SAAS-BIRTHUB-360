...
export function SDRSidebar() {
  ...
  const filteredTools = SDR_TOOLS.filter(
    (tool) =>
      ...
      <div className="border-b border-slate-200 p-4">
        <div className="mb-4 flex items-center gap-2 text-slate-800">
          ...
          <span className="text-lg font-bold tracking-tight">
            SDR <span className="text-blue-600">Commander</span>
          </span>
        </div>
        ...
        {filteredTools.map((tool) => {
          ...
          return (
            <button
              key={tool.id}
              onClick={() =>
                handleNavigation(`/dashboard/tool/${tool.id}`, tool.id)
              }
            ...
            >
              <div className="flex items-center gap-3 overflow-hidden">
                <div
                  className={`flex size-6 shrink-0 items-center justify-center rounded transition-colors ${isActive ? `text-${tool.color}-600` : 'text-slate-400 group-hover:text-slate-600'}`}
                >
