    ...
    <div className="mx-auto max-w-4xl py-10">
      <div className="mb-8 flex items-center justify-between">
        ...
        <div className="flex gap-2">
            <span className={`rounded-full px-3 py-1 text-xs font-bold ${step === 'DETAILS' ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-400'}`}>1. Details</span>
            <span className={`rounded-full px-3 py-1 text-xs font-bold ${step === 'AUDIENCE' ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-400'}`}>2. Audience</span>
            <span className={`rounded-full px-3 py-1 text-xs font-bold ${step === 'CONTENT' ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-400'}`}>3. Content</span>
            <span className={`rounded-full px-3 py-1 text-xs font-bold ${step === 'REVIEW' ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-400'}`}>4. Review</span>
        </div>
      </div>
        ...
        <CardContent className="space-y-6 p-6">
            ...
            {step === 'DETAILS' && (
                <div className="space-y-4">
                    ...
                    <div>
                        <label className="mb-1 block text-sm font-medium text-slate-300">Email Subject Line</label>
                        <Input
                            value={formData.subject}
                            onChange={(e) => setFormData({...formData, subject: e.target.value})}
                            placeholder="Quick question about {{companyName}}"
                        />
                        <p className="mt-1 text-xs text-slate-500">Tip: Use variables like {'{{firstName}}'} to personalize.</p>
                    </div>
                </div>
            )}
            ...
            {step === 'CONTENT' && (
                <div className="space-y-4">
                     <label className="mb-1 block text-sm font-medium text-slate-300">Email Body</label>
                     <EmailEditor
      ...
      <div className="mt-6 flex justify-between">
        ...
        {step !== 'REVIEW' ? (
            ...
            <form action={createCampaign}>
                ...
                <Button type="submit" className="bg-emerald-600 text-white hover:bg-emerald-500">Launch Campaign 🚀</Button>
            </form>
        )}
      </div>
    </div>
