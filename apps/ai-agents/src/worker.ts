...
export const emailWorker = createWorker<EmailJobData>(
  ...
  async (job: Job<EmailJobData>) => {
    ...
    const acquiredLock = await redisLock.set(
      lockKey,
      'processing',
      'EX',
      300,
    ...
    if (!acquiredLock) {
      logger.warn(
        { scheduledEmailId, jobId: job.id },
        'Idempotency lock active. Skipping duplicate job.',
      );
    ...
    try {
      ...
      if (emailRecord.status === 'SENT' || emailRecord.status === 'FAILED') {
        logger.info(
          { scheduledEmailId, status: emailRecord.status },
...
export async function scheduleEmails() {
  ...
  try {
    ...
    logger.info(
      ...
      'Found pending emails to schedule',
    );
...
createWorker(
  ...
  async (job) => {
    ...
    logger.info(
      { jobId: job.id, event, organizationId },
